export const nosPrestations = [
  {
    idService: "1",
    titlePresta: "Éducation du chiot et du jeune chien ",
    prix: "sur devis",
    detail:["Parce que l'éducation se commence dès le plus jeune âge, débuter  l'éducation de votre chiot sur de bonnes bases permet d'éviter le plus souvent des problèmes de communication et de comportement.",<br></br>,
    "En effet , l’arrivée d’un compagnon canin peut être déroutant, faire appel à un éducateur dès son plus jeune âge est un très bon réflexe. ",<br></br>,
   "Je vous apprendrez à avoir les bonnes attitudes et surtout à communiquer correctement afin d’avoir une harmonieuse avec votre loulou.",]
  },
  {
    idService: "2",
    titlePresta: "Rééducation du chien adulte et troubles de comportement ",
    prix: "Programme personnalisé ",
    detail:["Les problèmes de comportement type fugue, destruction, hyper-attachement, aboiements intempestifs, peur, agressivité, etc. ont des causes que je vous aiderai à identifier et pour lesquels je vous apporterai des solutions durables.",<br></br>,
    "Ma méthode de travail est basée sur l’éthologie canine.",<br></br>,
    "Je m’adapte à chaque chien en fonction de sa race et donc de ses aptitudes naturelles mais également en fonction de son âge et de son état de santé physique et psychologique. ",<br></br>,
    "Je travaille en collaboration avec une ostéopathe ce qui me permet d’avoir une vision médicale du chien que j’éduque.", <br></br>,
    "Chaque chien est unique même s'il existe des similitudes au sein d'une race.", <br></br>,
    "J’analyse , je prends en main votre chien et je vous explique. Mon but est de gagner la confiance de votre chien de manière à obtenir sa coopération en faisant appel à son intelligence d’où mon nom intelligence canine. ",<br></br>,
    "Le respect de  son intégrité physique et psychologique m’importe avant tout . Soyez en certains ♥️🐾🐾",]
  },
  {
    idService: "3",
    titlePresta: "Conseils pour vous aider à choisir votre compagnon à 4 pattes",
    prix: "sur demande",
    detail:"Conseils sur la race et tests  de caractère sur la portée.",
  },
  {
    idService: "4",
    titlePresta: "Tests de caractères à destination des éleveurs ",
    prix: "sur devis",
    detail:["Placer le bon chiot dans la bonne famille, un bonheur assuré pour tous.",<br></br>,
    "Déjà plus d’une centaine de tests réalisés avec une très grande satisfaction  des professionnels qui m’ont fait confiance",<br></br>,
    "Modalités organisationnelles", <br></br>,
    "Âge des chiots entre 5 et 7 semaines", <br></br>,
    "Un terrain ou une pièce non exploré par les chiots et cela en fonction des conditions climatiques, sera nécessaire pour la réalisation des tests.",<br></br>,
    "Tarifs appliqués et factures selon nombre de chiots et distance kilométrique parcourue",<br></br>,
    "Une fiche rédigée sur chaque chiot sera transmise à l’éleveur dans les 24h",<br></br>,
    "Le bon chiot dans la bonne famille pour ke bonheur de tous"],
  },
  
  {
    idService: "5",
    titlePresta: "Formation à la prévention morsure  pour  écoles , centres de loisirs ou sociétés",
    prix: "sur devis",
    detail:["Cette formation s’adresse aux enfants dans le cadre périscolaires, à des adultes dans le cadre de leur travail (livreurs, facteurs ..) ou de leur loisir (membres d’associations de sauvetage ou de loisirs  canins ) ." ,<br></br>,
    "Pourquoi ? ",<br></br>,
      "Éducateur canin comportementaliste, je suis très sensibilisée  à l’approche du chien par les enfants. En effet, les morsures de chiens sont la 3ème cause des accidents domestiques en France et 50% d’entre elles sont faites au visage chez les 6-10 ans. ",<br></br>,
      
     " Pour leur sécurité et le bien être de tous, il est indispensable d’éduquer les enfants au langage canin car comme pour la prévention routière , les bons gestes ça s’apprend. ",<br></br>,
      
      "Objectifs pédagogiques", <br></br>,
      "•Apprendre aux enfants à lire un chien de part sa posture, sa position de queue ",<br></br>,
      
     " •Comment prendre contact avec un chien inconnu",<br></br>,
      "Son approche, son langage ",<br></br>,
      
      "•Comment caresser correctement un chien sans lui envoyer de mauvais messages ",<br></br>,
      
      "•Comment réagir face à un chien agressif ou brutal ",<br></br>,
      
      "•Comment jouer avec un chien sans danger et dans le respect de l’animal afin de développer une bonne et saine complicité chien/enfant ",<br></br>,
           
      "Animation ",<br></br>,
    " Cette formation s’adresse essentiellement aux enfants de 6 à 11 ans  et aux adultes", <br></br>,
      "Elle dure entre 1h et 1h30 en fonction de la participation des enfants." ,<br></br>,
     " Son approche pédagogique est adaptée car ludique et interactive. Des petits exercices de mise en situations sont proposés aux enfants avec la présence de mon chien Nagano . ",<br></br>,
      
     " Côté organisationnel ",<br></br>,
      "Des groupes de 10 enfants maximum ",<br></br>,
      "Je peux intervenir au maximum 2 fois sur la journée pour une bonne gestion émotionnelle de mon chien.",<br></br>,

    ],

  },
  {
    idService: "6",
    titlePresta: "Conférence sur l’éthologie canine , la place du chien au sein de notre foyer ",
    prix: ["Tarif selon la thématique et pré inscription obligatoire,", <br></br>,
    "Infos sur page Facebook"],
    detail:["Deux thématiques:", <br></br>,
    "- Conférence sur l’éthologie canine , la place du chien au sein de notre foyer" ,<br></br>,
    "- stage communication canine en collaboration avec un maître expert en art martial qui vous enseignera des techniques de relaxation",]
    
  },
  {
    idService: "7",
    titlePresta: "Balade éducative, énigmatique et très ludique selon les délires de votre serviteur ",
    prix: "sur devis",
    detail:" ",
  },
  {
    idService: "8",
    titlePresta: "Prestations audios ",
    prix: "sur devis",
    detail:["Parce que l'éducation se commence dès le plus jeune âge, débuter  l'éducation de votre chiot sur de bonnes bases permet d'éviter le plus souvent des problèmes de communication et de comportement.",<br></br>,
    "En effet , l’arrivée d’un compagnon canin peut être déroutant, faire appel à un éducateur dès son plus jeune âge est un très bon réflexe.", <br></br>,
    "Je vous apprendrez à avoir les bonnes attitudes et surtout à communiquer correctement afin d’avoir une harmonieuse avec votre loulou.",<br></br>,
    
    "Dans cette formule je vous propose un accompagnement privilégié . En effet, je suis joignable par téléphone, mail entre les séances pour répondre à toutes vos questions, je mets à votre disposition des supports écrits pour vous permettre d'assimiler rapidement les idées essentielles et je peux également réaliser des vidéos de travail pour vous expliquer visuellement des exercices à distance ou vous corriger et conseiller sur les vidéos que vous me transmettez ."],
  },
  {
    idService: "9",
    titlePresta: "Déplacement en province",
    prix: "sur devis",
    detail:["Intelligence canine : une éducation canine intelligente et réfléchie",<br></br>,

    "Quelle que soit sa race, son gabarit, son âge, son tempérament, le chien est capable de vite nous cerner, de percevoir notre humeur, notre caractère et de s'adapter en fonction, de manière bien plus efficace que nous sommes en mesure de le faire à son égard.",<br></br>,
     
    "Il en résulte des 'malentendus' entre lui et l'humain pouvant déboucher sur des désagréments, voire des conflits : on ne comprend plus son chien et lui ne comprend pas nos attentes puisque nous ne parlons pas le même langage. ",<br></br>,
     
    "Je vais vous apprendre à communiquer avec votre compagnon à 4 pattes pour construire, développer ou rétablir une relation harmonieuse dans la durée.", <br></br>,
    
    "Quelque soit la problématique rencontrée je viendrais à votre rencontre"],
  },
];
